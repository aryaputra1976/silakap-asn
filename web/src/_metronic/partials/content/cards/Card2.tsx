...
const UsersList: FC<{users?: IconUserModel[]}> = ({users = []}) => {
  if (!users.length) {
    return null
  }
  ...
  return (
    <div className='symbol-group symbol-hover'>
      {users.map((user, index) => (
        <div key={`${user.name ?? 'user'}-${index}`} className='symbol symbol-35px symbol-circle'>
          {user.avatar ? (
            <img alt={user.name ?? 'User'} src={toAbsoluteUrl(user.avatar)} />
          ) : (
            <span className='symbol-label bg-light-primary text-primary fs-8 fw-bold'>
