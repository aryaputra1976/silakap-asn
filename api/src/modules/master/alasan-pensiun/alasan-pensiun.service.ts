import { Injectable, BadRequestException } from '@nestjs/common'
import { PrismaService } from '@/prisma/prisma.service'
import { BaseMasterService } from '@/modules/master/core/base-master.service'
import {
  AlasanPensiunRepository,
  AlasanPensiunEntity,
} from './alasan-pensiun.repository'
import { CreateAlasanPensiunDto } from './dto/create-alasan-pensiun.dto'
import { UpdateAlasanPensiunDto } from './dto/update-alasan-pensiun.dto'
import { QueryAlasanPensiunDto } from './dto/query-alasan-pensiun.dto'

@Injectable()
export class AlasanPensiunService extends BaseMasterService<
  AlasanPensiunEntity,
  CreateAlasanPensiunDto,
  UpdateAlasanPensiunDto,
  QueryAlasanPensiunDto
> {
  constructor(
    protected readonly repo: AlasanPensiunRepository,
    protected readonly prisma: PrismaService
  ) {
    super(repo, prisma)
  }

  protected async beforeCreateTx(
    tx: any,
    dto: CreateAlasanPensiunDto
  ): Promise<void> {
    await this.ensureNoDuplicateKodeTx(tx, dto.kode)
    await this.ensureNoDuplicateNamaTx(tx, dto.nama)
  }

  protected async beforeUpdateTx(
    tx: any,
    id: bigint,
    dto: UpdateAlasanPensiunDto
  ): Promise<void> {
    if (dto.nama) {
      const existing = await this.repo.findByNama(dto.nama, tx)

      if (existing && existing.id !== id) {
        throw new BadRequestException('Nama sudah digunakan')
      }
    }
  }

  private async ensureNoDuplicateKodeTx(
    tx: any,
    kode: string
  ) {
    const existing = await this.repo.findByKode(kode, tx)
    if (existing) {
      throw new BadRequestException('Kode sudah digunakan')
    }
  }

  private async ensureNoDuplicateNamaTx(
    tx: any,
    nama: string
  ) {
    const existing = await this.repo.findByNama(nama, tx)
    if (existing) {
      throw new BadRequestException('Nama sudah digunakan')
    }
  }
}